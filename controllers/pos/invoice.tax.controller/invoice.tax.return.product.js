...
const {
  ReturnProduct,
  validate,
} = require("../../../model/pos/preorder/return.product.model");
    ...
    ReturnProduct.find().then((value) => {
      ...
      if (!value) {
        ...
        if (findDate.length < 9) {
          res.send({
            status: true,
            invoice_return: `RE${dayjs(req.body.date).format("YYYYMM")}00000${
              findDate.length + 1
            }`,
          });
          ...
          res.send({
            status: true,
            invoice_return: `RE${dayjs(req.body.date).format("YYYYMM")}0000${
              findDate.length + 1
            }`,
          });
          ...
          res.send({
            status: true,
            invoice_return: `RE${dayjs(req.body.date).format("YYYYMM")}000${
              findDate.length + 1
            }`,
          });
          ...
          res.send({
            status: true,
            invoice_return: `RE${dayjs(req.body.date).format("YYYYMM")}00${
              findDate.length + 1
            }`,
          });
